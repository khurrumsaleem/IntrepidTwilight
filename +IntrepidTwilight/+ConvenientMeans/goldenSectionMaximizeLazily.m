function [pointMax,valueMax] = goldenSectionMaximizeLazily(f,interval,values,tolerance)
    ...
    %   If both interior samples are NOT less than the maximum of the end points,
    %   no search is performed.  Either the maximum of the function is the maximum
    ...
    if all(values([2,3]) > max(values([1,4])))
    ...
    valueMax = max(values)                  ;
    pointMax = points(values == valueMax)   ;
